...
function initGrid(){
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/ordercomplains/pageQuery'),
        ...
        columns: [
	        { display: '投诉人', name: 'userName',isSort: false,render:function(rowdata, rowindex, value){
	        	return MBIS.blank(rowdata['userName'],rowdata['loginName']);
	        }},
...
function toView(id){
	location.href=MBIS.U('admin/orderComplains/view','cid='+id);
}
function toHandle(id){
	location.href=MBIS.U('admin/orderComplains/toHandle','cid='+id);
}
function loadGrid(){
	var p = MBIS.arrayParams('.j-ipt');
	grid.set('url',MBIS.U('admin/orderComplains/pageQuery',p.join('&')));
}


function deliverNext(id){
     MBIS.confirm({content:'您确定要转交给应诉人应诉吗?',yes:function(){
       $.post(MBIS.U('Admin/Ordercomplains/deliverRespond'),{id:id},function(data,textStatus){
          var json = MBIS.toAdminJson(data);
          if(json.status=='1'){
        	  MBIS.msg('投诉已移交应诉人',{icon:1},function(){
        		  location.reload();
        	  });
          }else{
...
function finalHandle(id){
   ...
   if(params.finalResult==''){
     MBIS.msg('请输入仲裁结果!',{icon:2});
     return;
   }

   var c = MBIS.confirm({title:'信息提示',content:'您确定仲裁该订单投诉吗?',yes:function(){
     layer.close(c);
     $.post(MBIS.U('Admin/OrderComplains/finalHandle'),params,function(data,textStatus){
        var json = MBIS.toAdminJson(data);
        if(json.status=='1'){
