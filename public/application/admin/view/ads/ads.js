...
function initGrid(){
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/Ads/pageQuery'),
        ...
        columns: [
            ...
            var adFile = rowdata['adFile'].split(',');
              return'<img src="'+MBIS.conf.ROOT+'/'+adFile[0]+'" height="28px" />';
	        }},
          { display: '点击数', name: 'adClickNum', isSort: false},
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
	        	var h = "";
	            if(MBIS.GRANT.GGGL_02)h += "<a href='"+MBIS.U('admin/Ads/toEdit','id='+rowdata['adId'])+"'>修改</a> ";
	            if(MBIS.GRANT.GGGL_03)h += "<a href='javascript:toDel(" + rowdata['adId'] + ")'>删除</a> "; 
	            return h;
	        }}
        ]
    });
...
function toDel(id){
	var box = MBIS.confirm({content:"您确定要删除该记录吗?",yes:function(){
	           var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	           	$.post(MBIS.U('admin/Ads/del'),{id:id},function(data,textStatus){
	           			  layer.close(loading);
...
function doneChange(t,id){
  ...
  $.post(MBIS.U('admin/ads/changeSort'),{id:id,adSort:sort},function(data){
    var json = MBIS.toAdminJson(data);
    if(json.status==1){
        $(t).parent().attr('ondblclick','changeSort(this,'+id+')');
        $(t).parent().html(parseInt(sort));
...
function adsQuery(){
	    ...
	    grid.set('url',MBIS.U('admin/ads/pageQuery',query));
}
  ...
  //文件上传
	MBIS.upload({
      ...
      compress:false,//默认不对图片进行压缩
        ...
        var html = '<img src="'+MBIS.conf.ROOT+'/'+json.savePath+json.thumb+'" />';
        $('#preview').html(html);
        // 图片路径
 ...
 /* 表单验证 */
    $('#adsForm').validator({
          ...
          valid: function(form){
            ...
            var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
            $.post(MBIS.U('admin/Ads/'+((params.adId==0)?"add":"edit")),params,function(data,textStatus){
              layer.close(loading);
              var json = MBIS.toAdminJson(data);
              if(json.status=='1'){
                  MBIS.msg("操作成功",{icon:1});
                  location.href=MBIS.U('Admin/Ads/index');
              }else{
                    MBIS.msg(json.msg,{icon:2});
              }
            });
...
{
    $.post(MBIS.U('admin/Adpositions/getPositon'),{'positionType':pType},function(data,textStatus){
        positionInfo = data;
