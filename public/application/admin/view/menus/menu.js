...
$(function(){
	...
	var setting = {
	      ...
	      async: {
	           enable: true,
	           url:MBIS.U('admin/menus/listQuery'),
	...
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/privileges/listQuery'),
		pageSize:100,
		pageSizeOptions:[100],
		height:'99%',
        width:'100%',
        ...
        columns: [
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
	            var h = "";
	            if(MBIS.GRANT.QXGL_02)h += "<a href='javascript:getForEdit(" + rowdata['privilegeId'] + ")'>修改</a> ";
	            if(MBIS.GRANT.QXGL_03)h += "<a href='javascript:toDel(" + rowdata['privilegeId'] + ")'>删除</a> "; 
	            return h;
	        }}
        ]
    });
...
function onAsyncSuccess(event, treeId, treeNode, msg){
	var json = MBIS.toAdminJson(msg);
	console.log(treeNode)
	 if(treeNode != undefined && treeNode.level == 0){
          //调用默认展开第一个结点  
          var selectedNode = zTree.getSelectedNodes();  
          var nodes = zTree.getNodes(); 
          var nodes = nodes[0]['children'][0];
...
function onRightClick(event, treeId, treeNode) {
	...
	$('#'+treeNode.tId).bind('contextmenu',function(e){
		 var items = [];
		 if(MBIS.GRANT.CDGL_01)items.push({
			 icon : 'add',
             text : '新增菜单',
		 ...
		 treeNode = zTree.getSelectedNodes()[0];
		 if(treeNode.id>0){
			 if(MBIS.GRANT.CDGL_02)items.push({
                 icon  : 'pencil',
                 text  : '编辑菜单',
                 click : function(parent, menu) {
                  	treeNode = zTree.getSelectedNodes()[0];
			 ...
			 if(MBIS.GRANT.CDGL_03)items.push({
                 icon  : 'remove',
                 text  : '删除菜单',
                 click : function(parent, menu) {
                  	treeNode = zTree.getSelectedNodes()[0];
                  	layer.confirm('您确定要删除该菜单['+treeNode.name+']吗？', {btn: ['确定','取消']}, function(){
                  	    var loading = MBIS.msg('正在提交请求，请稍后...', {icon: 16,time:60000});
                  	    $.post(MBIS.U('admin/menus/del'),{id:treeNode.id},function(data,textStatus){
                  		     layer.close(loading);
...
function getForEditMenu(id){
    ...
    $.post(MBIS.U('admin/menus/get'),{id:id},function(data,textStatus){
          layer.close(loading);
          var json = MBIS.toAdminJson(data);
          if(json.menuId){
          	  editMenu(json);
...
function editMenu(obj){
	MBIS.setValues(obj);
	var box = MBIS.open({ title:(obj.menuId==0)?'新增菜单':"编辑菜单",type: 1,area: ['430px', '190px'],
	                content:$('#menuBox'),
	                btn:['确定','取消'],
	                yes: function(index, layero){
	                	if(!$('#menuName').isValid())return;
		                var params = MBIS.getParams('.ipt2');
		                params.menuId = obj.menuId;
		                var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
		                $.post(MBIS.U('admin/menus/'+((params.menuId)?"edit":"add")),params,function(data,textStatus){
		                	layer.close(loading);
		                	var json = MBIS.toAdminJson(data);
		                	if(json.status=='1'){
		                	    MBIS.msg("操作成功",{icon:1});
...
function getForEdit(id){
	 var loading = MBIS.msg('正在获取数据，请稍后...', {icon: 16,time:60000});
     $.post(MBIS.U('admin/privileges/get'),{id:id},function(data,textStatus){
           layer.close(loading);
           var json = MBIS.toAdminJson(data);
           if(json.privilegeId){
           		MBIS.setValues(json);
...
function toEdit(id){
	...
	var box = MBIS.open({title:title,type:1,content:$('#privilegeBox'),area: ['450px', '320px'],btn:['确定','取消'],yes:function(){
	                ...
	                params.id = id;
	                var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	           		$.post(MBIS.U('admin/privileges/'+((id==0)?"add":"edit")),params,function(data,textStatus){
	           			  layer.close(loading);
	           			  var json = MBIS.toAdminJson(data);
	           			  if(json.status=='1'){
	           			    	MBIS.msg("操作成功",{icon:1});
...
function toDel(id){
	var box = MBIS.confirm({content:"您确定要删除该权限吗?",yes:function(){
	           var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	           	$.post(MBIS.U('admin/privileges/del'),{id:id},function(data,textStatus){
	           			  layer.close(loading);
	           			  var json = MBIS.toAdminJson(data);
	           			  if(json.status=='1'){
	           			    	MBIS.msg("操作成功",{icon:1});
...
function checkPrivilegeCode(obj){
	...
	var loading = MBIS.msg('正在检测代码是否存在，请稍后...', {icon: 16,time:60000});
	$.post(MBIS.U('admin/privileges/checkPrivilegeCode'),{code:obj.value},function(data,textStatus){
		layer.close(loading);
		var json = MBIS.toAdminJson(data);
		if(json.status!='1'){
			MBIS.msg(json.msg,{icon:2});
