...
function initGrid(){
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/shops/pageQuery'),
        ...
        columns: [
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
	            var h = "";
	            if(MBIS.GRANT.DPGL_02)h += "<a href='javascript:toEdit(" + rowdata['shopId'] + ")'>修改</a> ";
	            if(MBIS.GRANT.DPGL_03)h += "<a href='javascript:toDel(" + rowdata['shopId'] + ")'>删除</a> "; 
	            return h;
	        }}
        ]
    });
...
function initStopGrid(){
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/shops/pageStopQuery'),
		pageSize:MBIS.pageSize,
		pageSizeOptions:MBIS.pageSizeOptions,
		height:'99%',
        width:'100%',
...
function initEdit(opts){
	MBIS.upload({
	  	  ...
	  	  callback:function(f){
	            ...
	            $('#preview').attr('src',MBIS.conf.ROOT+"/"+json.savePath+json.thumb);
...
function toEdit(id){
	location.href=MBIS.U('admin/shops/toEdit','id='+id);
}
function toDel(id){
	var box = MBIS.confirm({content:"您确定要删除该店铺吗?",yes:function(){
	           var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	           $.post(MBIS.U('admin/shops/del'),{id:id},function(data,textStatus){
	           			  layer.close(loading);
...
function checkLoginKey(obj){
	...
	var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(MBIS.U('admin/users/checkLoginKey'),params,function(data,textStatus){
    	layer.close(loading);
    	var json = MBIS.toAdminJson(data);
    	if(json.status!='1'){
    		MBIS.msg(json.msg,{icon:2});
...
function save(){
	$('#editFrom').isValid(function(v){
		if(v){
			...
			params.areaId = MBIS.ITGetAreaVal('j-areas');
			params.bankAreaId = MBIS.ITGetAreaVal('j-bareas');
			var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
		    $.post(MBIS.U('admin/shops/'+((params.shopId==0)?"add":"edit")),params,function(data,textStatus){
		    	layer.close(loading);
		    	var json = MBIS.toAdminJson(data);
		    	if(json.status=='1'){
		    		MBIS.msg("操作成功",{icon:1});
		    		location.href=MBIS.U('admin/shops/index');
		    	}else{
		    		MBIS.msg(json.msg,{icon:2});
		    	}
		    });
