...
function initGrid(){
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/goodsappraises/pageQuery'),
		pageSize:100,
		pageSizeOptions:[100],
		height:'99%',
        width:'100%',
        ...
        columns: [
	        ...
	        { display: '商品主图', name: 'goodsImg', isSort: false,render:function(rowdata, rowindex, value){
	        	...
	        	return "<img src='"+MBIS.conf.ROOT+"/"+thumb+"' height='28' width='28'/>";
	        	
	        }},
	        { display: '商品评分', name: 'goodsScore', isSort: false,render:function(rowdata, rowindex, value){
	        	var s="<div style='line-height:28px;'>";
	        	for(var i=0;i<value;++i){
	        		s +="<img src='"+MBIS.conf.ROOT+"/application/admin/view/goodsappraises/icon_score_yes.png'>";
	        	}
	        	s += "</div>";
	        	return s;
	        }},
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
	            var h = "";
	            if(MBIS.GRANT.PJGL_02)h += "<a href='"+MBIS.U('admin/goodsappraises/toEdit','id='+rowdata['id'])+"'>修改</a> ";
	            if(MBIS.GRANT.PJGL_03)h += "<a href='javascript:toDel(" + rowdata['id'] + ")'>删除</a> "; 
	            return h;
	        }}
        ]
    });
...
function toDel(id){
	var box = MBIS.confirm({content:"您确定要删除该记录吗?",yes:function(){
	           var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	           	$.post(MBIS.U('admin/goodsappraises/del'),{id:id},function(data,textStatus){
	           			  layer.close(loading);
...
function loadGrid1(){
	    ...
	    grid.set('url',MBIS.U('admin/goodsappraises/pageQuery',query));
}
...
/* 表单验证 */
    $('#goodsAppraisesForm').validator({
          ...
          valid: function(form){
            ...
            var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
            $.post(MBIS.U('admin/goodsappraises/'+((params.id==0)?"add":"edit")),params,function(data,textStatus){
              layer.close(loading);
              var json = MBIS.toAdminJson(data);
              if(json.status=='1'){
                  MBIS.msg("操作成功",{icon:1});
                  location.href=MBIS.U('Admin/goodsappraises/index');
              }else{
                    MBIS.msg(json.msg,{icon:2});
              }
            });
