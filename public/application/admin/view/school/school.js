...
function initGrid(type_id){
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/school/pageQuery','type_id='+type_id),
        ...
        columns: [
            ...
            { display: '校长',name: 'principal_name',width: 100, isSort: false},
	        { display: '电话', name: 'principal_mobile',width: 200,isSort: false},
...
function loadGrid(type_id){
	grid.set('url',MBIS.U('admin/school/pageQuery','key='+$('#key').val()+'&type_id='+type_id));
}
function refresh(type_id){
	if($('#key').val() !== ''){
		$('#key').val('');
	}
    grid.set('url',MBIS.U('admin/school/pageQuery','type_id='+type_id));
}
