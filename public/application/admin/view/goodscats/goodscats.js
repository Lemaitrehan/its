var grid;
function initGrid(){	
	grid = $('#maingrid').MBISGridTree({
		url:MBIS.U('admin/goodscats/pageQuery'),
		pageSize:10000,
        ...
        columns: [
	        ...
	        { display: '操作', name: 'op',width: 150,isSort: false,
	        	render: function (rowdata){
		            var h = "";
			        if(MBIS.GRANT.SPFL_01)h += "<a href='javascript:toEdit("+rowdata["catId"]+",0)'>新增子分类</a> ";
		            if(MBIS.GRANT.SPFL_02)h += "<a href='javascript:toEdit("+rowdata["parentId"]+","+rowdata["catId"]+")'>修改</a> ";
		            if(MBIS.GRANT.SPFL_03)h += "<a href='javascript:toDel("+rowdata["parentId"]+","+rowdata["catId"]+")'>删除</a> "; 
		            return h;
	        	}}
        ]
    });
...
function toggleIsFloor(obj,id){
	if(!MBIS.GRANT.SPFL_02)return;
    var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    var v = ($(obj).attr('v')=='1')?0:1;
	$.post(MBIS.U('admin/goodscats/editiIsFloor'),{id:id,isFloor:v},function(data,textStatus){
		  layer.close(loading);
...
function toggleIsShow(obj,id){
    ...
    var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    var v = ($(obj).attr('v')=='1')?0:1;
    $.post(MBIS.U('admin/goodscats/editiIsShow'),{id:id,isShow:v},function(data,textStatus){
		layer.close(loading);
		var json = MBIS.toAdminJson(data);
		if(json.status=='1'){
			 MBIS.msg(json.msg,{icon:1});
...
function toEdit(pid,id){
	...
	if(id>0){
		$.post(MBIS.U('admin/goodscats/get'),{id:id},function(data,textStatus){
			var json = MBIS.toAdminJson(data);
			if(json){
				MBIS.setValues(json);
				editsBox(id);
		...
		MBIS.setValues({parentId:pid,catName:'',isShow:1,isFloor:0,catSort:0});
		editsBox(id);
	}
}
...
function editsBox(id,v){
	...
	var box = MBIS.open({title:title,type:1,content:$('#goodscatsBox'),area: ['465px', '300px'],btn:['确定','取消'],yes:function(){
	...
	$('#goodscatsForm').validator({
	    ...
	    valid: function(form){
	        ...
	        params.id = id;
	        var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    		$.post(MBIS.U('admin/goodscats/'+((id>0)?"edit":"add")),params,function(data,textStatus){
    			  layer.close(loading);
    			  var json = MBIS.toAdminJson(data);
    			  if(json.status=='1'){
    			    	MBIS.msg(json.msg,{icon:1});
...
function toDel(pid,id){
	var box = MBIS.confirm({content:"您确定要删除该商品分类吗?",yes:function(){
	           var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	           	$.post(MBIS.U('admin/goodscats/del'),{id:id},function(data,textStatus){
	           			  layer.close(loading);
	           			  var json = MBIS.toAdminJson(data);
	           			  if(json.status=='1'){
	           			       MBIS.msg(json.msg,{icon:1});
