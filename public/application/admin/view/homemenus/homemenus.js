...
$(function(){
	...
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/homemenus/pageQuery'),
        ...
        columns: [
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
	            var h = "";
	            if(MBIS.GRANT.QTCD_01)h += "<a href='javascript:toEdit(0," + rowdata['menuId'] + ")'>添加子菜单</a> ";
	            if(MBIS.GRANT.QTCD_02)h += "<a href='javascript:getForEdit(" + rowdata['menuId'] + ")' href='"+MBIS.U('admin/homemenus/toEdit','menuId='+rowdata['menuId'])+"'>修改</a> ";
	            if(MBIS.GRANT.QTCD_03)h += "<a href='javascript:toDel(" + rowdata['menuId'] + ")'>删除</a> "; 
	            return h;
	        }}
        ],
...
function doneChange(t,id){
  ...
  $.post(MBIS.U('admin/homemenus/changeSort'),{id:id,menuSort:sort},function(data){
...
function toDel(menuId){
	var box = MBIS.confirm({content:"删除该菜单会将下边的子菜单也一并删除，您确定要删除吗?",yes:function(){
		var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
		$.post(MBIS.U('admin/homemenus/del'),{menuId:menuId},function(data,textStatus){
			layer.close(loading);
			var json = MBIS.toAdminJson(data);
			if(json.status=='1'){
...
function edit(menuId){
  ...
  var params = MBIS.getParams('.ipt');
    ...
    var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(MBIS.U('admin/homemenus/'+((menuId==0)?"add":"edit")),params,function(data,textStatus){
      layer.close(loading);
      var json = MBIS.toAdminJson(data);
      if(json.status=='1'){
          MBIS.msg("操作成功",{icon:1});
          location.href=MBIS.U('admin/homemenus/index');
      }else{
            MBIS.msg(json.msg,{icon:2});
      }
    });
...
function isShowtoggle(menuId, isShow){
  if(!MBIS.GRANT.QTCD_02)return;
	$.post(MBIS.U('admin/homemenus/setToggle'), {'menuId':menuId, 'isShow':isShow}, function(data, textStatus){
		var json = MBIS.toAdminJson(data);
		if(json.status=='1'){
			MBIS.msg("操作成功",{icon:1});
			grid.reload();
...
function getForEdit(menuId){
    ...
    $.post(MBIS.U('admin/homemenus/get'),{menuId:menuId},function(data,textStatus){
          layer.close(loading);
          var json = MBIS.toAdminJson(data);
          if(json.menuId){
          		MBIS.setValues(json);
...
function toEdit(menuId,parentId){
	...
	var box = MBIS.open({title:title,type:1,content:$('#menuBox'),area: ['550px', '350px'],btn:['确定','取消'],yes:function(){
		$('#menuForm').submit();
	}});
	$('#menuForm').validator({
        fields: {
...
function loadGrid(){
	grid.set('url',MBIS.U('admin/homemenus/pageQuery','menuType='+$('#s_menuType').val()));
}