...
function initGrid(){
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/settlements/pageQuery'),
        ...
        columns: [
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
	            ...
	            if(rowdata['settlementStatus']==0 && MBIS.GRANT.JSSQ_04)h += "<a href='javascript:toEdit(" + rowdata['settlementId'] + ")'>处理</a> ";
	            return h;
	        }}
        ]
    });
...
function toEdit(id){
	location.href=MBIS.U('admin/settlements/toHandle','id='+id);
}
function toView(id){
	location.href=MBIS.U('admin/settlements/toView','id='+id);
}
function loadGrid(){
	grid.set('url',MBIS.U('admin/settlements/pageQuery','settlementNo='+$('#settlementNo').val()+"&settlementStatus="+$('#settlementStatus').val()+"&shopName="+$('#shopName').val()));
}

function save(){
	if(MBIS.confirm({content:'您确定提交该结算单吗？',yes:function(){
        var params = MBIS.getParams('.ipt');
		var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	    $.post(MBIS.U('admin/settlements/handle'),params,function(data,textStatus){
	    	layer.close(loading);
	    	var json = MBIS.toAdminJson(data);
	    	if(json.status=='1'){
	    		MBIS.msg("操作成功",{icon:1});
	    		location.href=MBIS.U('admin/settlements/index');
	    	}else{
...
function initGoodsGrid(id){
	$('#wst-tab-2').height(MBIS.pageHeight()-25);
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/settlements/pageGoodsQuery','id='+id),
		pageSize:MBIS.pageSize,
		pageSizeOptions:MBIS.pageSizeOptions,
		height:'99%',
        width:'100%',
