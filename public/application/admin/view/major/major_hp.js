var grid;
var combo;
function initGrid(type_id,school_id){
    var grid_config = {
		url:MBIS.U('admin/major/pageQuery','type_id='+type_id),
...
//    if(type_id == 1)
//    {
//        grid_config.columns.push({ display: '层次',name: 'level_type',width: 100, isSort: false});
//        grid_config.columns.push({ display: '考试类型', name: 'exam_type',width: 100,isSort: false});
//        grid_config.columns.push({ display: '毕业时间', name: 'graduate_type',width: 100,isSort: false});
//        grid_config.columns.push({ display: '学历类型', name: 'edu_type',width: 100,isSort: false});
//    }
    grid_config.columns.push({ display: '操作', name: 'op',width: 200,isSort: false,
	        	render: function (rowdata){
                    ...
                    if(type_id == 1)
                    {
                        if(rowdata['exam_type']=='自考'){
                        	if(MBIS.GRANT.XLZY_05)h += "<a href='"+MBIS.U('admin/subject/index')+"?type_id="+type_id+"&major_id="+rowdata["major_id"]+"'>科目列表</a> ";
                        }
                        if(MBIS.GRANT.XLZY_04)h += "<a href='"+MBIS.U('admin/grade/index')+"?major_id="+rowdata["major_id"]+"'>编辑</a> ";
		            	if(MBIS.GRANT.XLZY_02)h += "<a href='javascript:toEdit("+type_id+","+rowdata["major_id"]+")'>查看</a> ";
		            	if(MBIS.GRANT.XLZY_03)h += "<a href='javascript:toDel("+rowdata["major_id"]+")'>删除</a> ";
		            }else
		            if(type_id == 2){
		            	if(MBIS.GRANT.ZYSJ_02)h += "<a href='javascript:toEdit("+type_id+","+rowdata["major_id"]+")'>修改</a> ";
...
function refresh(type_id){
    ...
    grid.set('url',MBIS.U('admin/major/pageQuery','type_id='+type_id));
}
function majorQuery(){
...
function toEdit(type_id,id){
	location.href=MBIS.U('admin/major/toEdit','id='+id+'&type_id='+type_id);
}
...
function toDel(id){
	var box = MBIS.confirm({content:"您确定要删除该数据吗?",yes:function(){
	           var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	           	$.post(MBIS.U('admin/major/del'),{id:id},function(data,textStatus){
	           			  layer.close(loading);
	           			  var json = MBIS.toAdminJson(data);
	           			  if(json.status=='1'){
	           			    	MBIS.msg(json.msg,{icon:1});
