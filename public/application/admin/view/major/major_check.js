var grid;
var combo;
function initGrid(major_id){
    var grid_config = {
		url:MBIS.U('admin/major/checkList','major_id='+major_id),
		...
		height:'99%',
                ...
                rownumbers:true,
                columns: [
            { display: '专业编号', name: 'major_num',isSort: false},
            { display: '专业名称', name: 'name',isSort: false},
            { display: '考试类型', name: 'exam_type',isSort: false},
            { display: '层次', name: 'level',isSort: false},
            { display: '毕业时间', name: 'graduate_time',isSort: false},
            { display: '学历类型', name: 'type',isSort: false},
            { display: '关联院校', name: 'school_name',isSort: false},
        ]
    };
        grid_config.columns.push({ display: '操作', name: 'op',width: 200,isSort: false,
	        	render: function (rowdata){
		            var h = "";
		            	if(MBIS.GRANT.XLZY_02)h += "<a href='javascript:toCheck("+rowdata["major_id"]+','+rowdata['level_id']+")'>科目</a> ";
		            	if(MBIS.GRANT.XLZY_03)h += "<a href='javascript:toDel("+rowdata["major_num"]+")'>历史价格</a> ";
...
function toCheck(major_id,level){
    location.href=MBIS.U('admin/major/selectSubjectlist','major_id='+major_id+'&level='+level);
}
