var grid;
function toView(id){
	location.href=MBIS.U('admin/orders/view','id='+id);
}
function initRefundGrid(){
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/orderrefunds/refundPageQuery'),
        ...
        columns: [
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
	            ...
	            if(rowdata['isRefund']==0){
	            	if(MBIS.GRANT.TKDD_04)h += "<a href='javascript:toRefund(" + rowdata['refundId'] + ")'>退款</a> ";
	            }
...
function loadRefundGrid(){
	...
	grid.set('url',MBIS.U('admin/orderrefunds/refundPageQuery',p.join('&')));
}
var w;
function toRefund(id){
	var ll = MBIS.msg('正在加载信息，请稍候...');
	$.post(MBIS.U('admin/orderrefunds/toRefund',{id:id}),{},function(data){
		layer.close(ll);
		w =MBIS.open({type: 1,title:"订单退款",shade: [0.6, '#000'],offset:'50px',border: [0],content:data,area: ['550px', '370px']});
	});
}
function orderRefund(id){
	$('#editFrom').isValid(function(v){
		if(v){
        	...
        	params.id = id;
        	ll = MBIS.msg('正在加载信息，请稍候...');
		    $.post(MBIS.U('admin/orderrefunds/orderRefund'),params,function(data){
		    	layer.close(ll);
