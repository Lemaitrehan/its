...
function loadGoods(suffix){
	var params = MBIS.getParams('.ipt'+suffix);
	params.key = params['key'+suffix];
	params.goodsCatId = MBIS.ITGetGoodsCatVal('pgoodsCats1'+suffix);
	if(params.goodsCatId==''){
		MBIS.msg('请选择一个商品分类',{icon:2});
		return;
	}
	var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(MBIS.U('admin/recommends/searchGoods'),params,function(data,textStatus){
    	layer.close(loading);
...
function listQueryByGoods(suffix){
	...
	params.goodsCatId = MBIS.ITGetGoodsCatVal('pgoodsCats2'+suffix);
	var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(MBIS.U('admin/recommends/listQueryByGoods'),params,function(data,textStatus){
    	layer.close(loading);
    	var json = MBIS.toAdminJson(data);
    	if(json.status=='1'){
    		...
    		if(MBIS.ITGetGoodsCatVal('pgoodsCats1'+suffix)>0)loadGoods(suffix);
    	}
    });
}
function editGoods(suffix){
	...
	params.goodsCatId = MBIS.ITGetGoodsCatVal('pgoodsCats2'+suffix);
	var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(MBIS.U('admin/recommends/editGoods'),params,function(data,textStatus){
    	layer.close(loading);
    	var json = MBIS.toAdminJson(data);
    	if(json.status=='1'){
    		MBIS.msg("保存成功",{icon:1});
    	}else{
    		MBIS.msg(json.msg,{icon:2});
    	}
    });
...
function loadShops(suffix){
	...
	params.goodsCatId = MBIS.ITGetGoodsCatVal('pgoodsCats1'+suffix);
	if(params.goodsCatId==''){
		MBIS.msg('请选择一个经营范围',{icon:2});
		return;
	}
	var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(MBIS.U('admin/recommends/searchShops'),params,function(data,textStatus){
    	layer.close(loading);
    	var json = MBIS.toAdminJson(data);
    	if(json.status=='1'){
    		if(!json.data)return;
...
function listQueryByShops(suffix){
	...
	params.goodsCatId = MBIS.ITGetGoodsCatVal('pgoodsCats2'+suffix);
	var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(MBIS.U('admin/recommends/listQueryByShops'),params,function(data,textStatus){
    	layer.close(loading);
    	var json = MBIS.toAdminJson(data);
    	if(json.status=='1'){
    		if(json.data && json.data.length){
    		...
    		if(MBIS.ITGetGoodsCatVal('pgoodsCats1'+suffix)>0)loadShops(suffix);
    	}
    });
}
...
function editShops(suffix){
	...
	params.goodsCatId = MBIS.ITGetGoodsCatVal('pgoodsCats2'+suffix);
	var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(MBIS.U('admin/recommends/editShops'),params,function(data,textStatus){
    	layer.close(loading);
    	var json = MBIS.toAdminJson(data);
    	if(json.status=='1'){
    		MBIS.msg("保存成功",{icon:1});
...
function loadBrands(suffix){
	...
	var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(MBIS.U('admin/recommends/searchBrands'),params,function(data,textStatus){
    	layer.close(loading);
    	var json = MBIS.toAdminJson(data);
    	if(json.status=='1'){
    		if(!json.data)return;
...
function listQueryByBrands(suffix){
	...
	params.goodsCatId = MBIS.ITGetGoodsCatVal('pgoodsCats2'+suffix);
	var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(MBIS.U('admin/recommends/listQueryByBrands'),params,function(data,textStatus){
    	layer.close(loading);
    	var json = MBIS.toAdminJson(data);
    	if(json.status=='1'){
    		if(json.data && json.data.length){
    		...
    		if(MBIS.ITGetGoodsCatVal('pgoodsCats1'+suffix)>0)loadBrands(suffix);
    	}
    });
}
...
function editBrands(suffix){
	...
	params.goodsCatId = MBIS.ITGetGoodsCatVal('pgoodsCats2'+suffix);
	var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    $.post(MBIS.U('admin/recommends/editBrands'),params,function(data,textStatus){
    	layer.close(loading);
    	var json = MBIS.toAdminJson(data);
    	if(json.status=='1'){
    		MBIS.msg("保存成功",{icon:1});
