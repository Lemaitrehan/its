...
function initGrid(){
	...
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/Messages/pageQuery'),
        ...
        columns: [
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
	        	var h="";
	            if(MBIS.GRANT.SCXX_00)h += "<a href='javascript:showFullMsg(" + rowdata['id'] + ")'>查看</a> ";
	            if(MBIS.GRANT.SCXX_03)h += "<a href='javascript:toDel(" + rowdata['id'] + ")'>删除</a> "; 
	            return h;
	        }}
        ]
    });
...
function showFullMsg(id){
	parent.showBox({title:'内容详情',type:2,content:MBIS.U('admin/messages/showFullMsg','id='+id),area: ['800px', '500px'],btn:['关闭']});
...
function toDel(id){
	var box = MBIS.confirm({content:"您确定要删除该记录吗?",yes:function(){
	           var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	           	$.post(MBIS.U('admin/messages/del'),{id:id},function(data,textStatus){
	           			  layer.close(loading);
...
editor1 = K.create('textarea[name="msgContent"]', {
  uploadJson : MBIS.conf.ROOT+'/admin/messages/editorUpload',
  height:'350px',
...
function sendToTheUser(t){
     ...
     function userQuery(){
      ...
      var html = '';
      $.post(MBIS.U('admin/messages/userQuery'),{'loginName':key},function(text,dataStatus){
     ...
     function sendMsg(){
        ...
        var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
        $.post(MBIS.U('admin/messages/add'),params,function(data,textStatus){
          layer.close(loading);
          var json = MBIS.toAdminJson(data);
          if(json.status=='1'){
              MBIS.msg("操作成功",{icon:1});
...
function msgQuery(){
    var query = MBIS.getParams('.query');
      grid.set('url',MBIS.U('admin/messages/pageQuery',query));
  }
