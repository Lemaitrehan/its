...
function initGrid(){
	grid = $("#maingrid").ligerGrid({
		url:MBIS.U('admin/faq/pageQuery'),
        ...
        rownumbers:true,
        columns: [
	        { display: '帮助主题', name: 'articleTitle',isSort: false},
	        ...
	        { display: '操作', name: 'op',width: 100,isSort: false,
	        	render: function (rowdata){
		            var h = "";
                    h += "<a href='javascript:toView("+rowdata["articleId"]+")'>查看</a> ";
		            if(MBIS.GRANT.WTJD_02)h += "<a href='javascript:toEdit("+rowdata["articleId"]+")'>修改</a> ";
...
function loadGrid(){
	grid.set('url',MBIS.U('admin/faq/pageQuery','key='+$('#key').val()));
}

function toggleIsShow(t,v){
	if(!MBIS.GRANT.WZGL_02)return;
    var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
    	$.post(MBIS.U('admin/faq/editiIsShow'),{id:v,isShow:t},function(data,textStatus){
			  layer.close(loading);
...
function toEdit(id){
	location.href=MBIS.U('admin/faq/toEdit','id='+id);
}

function toView(id){
	location.href=MBIS.U('admin/faq/toView','id='+id);
}
...
function toEdits(id){
    ...
    params.id = id;
    var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	$.post(MBIS.U('admin/faq/'+((id>0)?"edit":"add")),params,function(data,textStatus){
		  layer.close(loading);
		  var json = MBIS.toAdminJson(data);
		  if(json.status=='1'){
		    	MBIS.msg(json.msg,{icon:1});
		        setTimeout(function(){ 
			    	location.href=MBIS.U('admin/faq/index');
		        },1000);
		  }else{
		        MBIS.msg(json.msg,{icon:2});
		  }
...
function toDel(id){
	var box = MBIS.confirm({content:"您确定要删除该文章吗?",yes:function(){
	           var loading = MBIS.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
	           	$.post(MBIS.U('admin/faq/del'),{id:id},function(data,textStatus){
	           			  layer.close(loading);
	           			  var json = MBIS.toAdminJson(data);
	           			  if(json.status=='1'){
	           			    	MBIS.msg(json.msg,{icon:1});
