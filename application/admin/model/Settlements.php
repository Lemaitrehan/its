...
class Settlements extends Base{
	...
	public function handle(){
        ...
        try{
			...
			if(false !== $rs){
				...
				MBISSendMsg($shop['userId'],"您的结算申请【".$object->settlementNo."】已处理，请留意到账户息哦~",['from'=>4,'dataId'=>$id]);
   ...
   public function pageShopOrderQuery(){
        ...
        if(count($page['Rows'])>0){
        	foreach ($page['Rows'] as $key => $v) {
        		$page['Rows'][$key]['payTypeName'] = MBISLangPayType($v['payType']);
        	}
        }
        return $page;
   }
    ...
    */
	public function generateSettleByShop(){
    	...
    	if(empty($orders))return MBISReturn('没有需要结算的订单，请刷新后再核对!');
    	$settlementMoney = 0;
    	...
    	if(empty($shops))MBISReturn('无效的店铺结算账号!');
    	Db::startTrans();
		try{
            ...
            if(false !==  $result){
                ...
                Db::execute($upSql);
                //发消息
				MBISSendMsg($shops['userId'],"您有新的结算单【".$this->settlementNo."】生成，请留意结算信息~",['from'=>4,'dataId'=>$this->settlementId]);
				//增加一条资金变动信息
				$lm = [];
				$lm['targetType'] = 1;
				$lm['targetId'] = $shopId;
				...
				model('LogMoneys')->save($lm);
				Db::commit();
            	return MBISReturn('生成结算单成功',1);
            }
		}catch (\Exception $e) {
            print_r($e);
            Db::rollback();
        }
        return MBISReturn('生成结算单失败',-1);
    }
}