...
class OrderRefunds extends Base{
	...
	public function refundPageQuery(){
	    ...
	    if(count($page['Rows'])>0){
	    	 foreach ($page['Rows'] as $key => $v){
	    	 	 $page['Rows'][$key]['payType'] = MBISLangPayType($v['payType']);
	    	 	 $page['Rows'][$key]['deliverType'] = MBISLangDeliverType($v['deliverType']==1);
	    	 	 $page['Rows'][$key]['status'] = MBISLangOrderStatus($v['orderStatus']);
	    	 }
	    }
	    return $page;
	}
	...
	public function orderRefund(){
		...
		if($id==0)return MBISReturn("操作失败!");
		$refund = $this->get($id);
		if(empty($refund) || $refund->refundStatus!=1)return MBISReturn("该退款订单不存在或已退款!");
		Db::startTrans();
        try{
        	$order = model('orders')->get($refund->orderId);
        	if(!in_array($order->orderStatus,[-1,-3]) || $order->payType !=1)return MBISReturn("无效的退款订单!");
			//修改退款单信息
			...
			//发送一条用户信息
			MBISSendMsg($order->userId,"您的退款订单【".$order->orderNo."】已处理，请留意账户到账情况。".(($content!='')?"【退款备注：".$content."】":""),['from'=>1,'dataId'=>$order->orderId]);
			//如果有钱剩下，那么就退回到商家钱包
			...
			if($shopMoneys>0){
				...
				Db::name('log_moneys')->insert($lm);
				//发送商家信息
				MBISSendMsg($shop->userId,"退款订单【".$order->orderNo."】已通过，返回商家金额¥".$shopMoneys."，请留意账户情况。",['from'=>1,'dataId'=>$order->orderId]);
			}
			Db::commit();
		...
		return MBISReturn("操作失败，请刷新后再重试"); 
	}
}
