...
class Shops extends Base{
    ...
    public function del(){
	    ...
	    MBISUnuseImage('shops','shopImg',$id);
        if(false !== $result){
	...
	public function add(){
		...
		if($userId==0){
			...
			$ck = MBISCheckLoginKey($user['loginName']);
        ...
        if(!$validate->check(Input('post.')))return MBISReturn($validate->getError());
        //判断经营范围
        $goodsCatIds = Input('post.goodsCatIds');
        $accredIds = Input('post.accredIds');
        if($goodsCatIds=='')return MBISReturn('请选择经营范围');
        Db::startTrans();
        try{
        	//如果是游客的话就先新增会员资料
        	if($userId==0){
	        ...
	        MBISUnset($data,'shopId,dataFlag,isSelf');
	        ...
	        if($userId>0){
	        	...
	        	$shopId = $this->shopId;
	        	//启用上传图片
				MBISUseImages(1, $shopId, $data['shopImg']);
	        	//建立店铺配置信息
	        	$sc = [];
	        	$sc['shopId'] = $shopId;
	...
	public function edit(){
        ...
        try{
		    ...
		    $areaIds = model('Areas')->getParentIs($data['bankAreaId']);
		    if(!empty($areaIds))$data['bankAreaIdPath'] = implode('_',$areaIds)."_";
	        MBISUnset($data,'shopId,userId,dataFlag,createTime,applyId,goodsCatIds,accredIds,isSelf');
	        //启用上传图片
			MBISUseImages(1, $shopId, $data['shopImg'],'shops','shopImg');
