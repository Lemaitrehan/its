...
use think\Db;
use application\admin\model\Users as Custom;
/**
 * 客户业务处理
 */
class Customuser extends Base{
	...
	public function pageQueryC(){
		$User = new Custom();
		$where = [];
		$where ['uidType']  = ['in',[1,2]];	//uidType 1,2为客户;3为会员
		$where ['userType']	= ['=',0];		//userType 0为学员;1为老师;2为咨询师
		...
		*/
		...
		$field = 'u.userId,u.loginName,u.user_weixin,u.idcard,u.student_no,u.trueName,u.userPhone,u.userQQ,u.userEmail,x.company,x.urgency_contact,x.urgency_contact_mobile';
	...
	public function getCustomInfo($id,$key=''){	//根据id获取相关数据信息
		$User = new Custom();
		$where = [];
		$where ['u.userId'] = ['=',"$id"];
	...
	public function delCustom(){	//删除操作
		...
		try{
	        ...
	        if(false !== $result){
	        	model('studentExtend')->where(['userId'=>$id])->delete();  //扩展信息
	        	Db::commit();
