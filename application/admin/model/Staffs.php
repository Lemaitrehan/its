...
class Staffs extends Base{
	...
	public function checkLogin(){
		...
		if(empty($staff))return MBISReturn('账号或密码错误!');
		if($staff['loginPwd']==md5($loginPwd.$staff['secretKey'])){
			...
			return MBISReturn("",1,$staff);
		}
		return MBISReturn('账号或密码错误!');
	}
    ...
    public function del(){
		...
		try{
	        ...
	        if(false !== $result){
	        	MBISUnuseImage('staffs','staffPhoto',$id);
	        	Db::commit();
	...
	public function add(){
		...
		try{
		   ...
		   if(false !== $result){
		      MBISUseImages(1, $this->staffId,$data['staffPhoto']);
		      Db::commit();
	...
	public function edit(){
		...
		$data = input('post.');
		MBISUnset($data, 'staffId,loginPwd,secretKey,dataFlag,createTime,lastTime,lastIP');
		Db::startTrans();
		try{
			MBISUseImages(1, $id,$data['staffPhoto'],'staffs','staffPhoto');
	...
	public function checkLoginKey($key){
		...
		return ($rs==0)?MBISReturn('该账号可用', 1):MBISReturn("对不起，该账号已存在");
	}
	/**
	 * 修改自己密码
	 */
	public function editMyPass($staffId){
		if(input("post.newPass")=='')MBISReturn("密码不能为空");
		$staff = $this->where('staffId',$staffId)->field('secretKey,loginPwd')->find();
		if(empty($staff))return MBISReturn("修改失败");
		$srcPass = md5(input("post.srcPass").$staff["secretKey"]);
		if($srcPass!=$staff['loginPwd'])return MBISReturn("原密码错误");
