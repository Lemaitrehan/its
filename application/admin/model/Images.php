...
class Images extends Base{
	...
	public function del(){
		...
		if(false !== $rs){
			...
			if(file_exists(MBISRootPath()."/".$image['imgPath']))unlink(MBISRootPath()."/".$image['imgPath']); 
			if(file_exists(MBISRootPath()."/".$timgPath))unlink(MBISRootPath()."/".$timgPath); 
			if(file_exists(MBISRootPath()."/".$mimgPath))unlink(MBISRootPath()."/".$mimgPath); 
			if(file_exists(MBISRootPath()."/".$mtimgPath))unlink(MBISRootPath()."/".$mtimgPath); 
			return MBISReturn("删除成功", 1);
		}
		return MBISReturn("删除失败");
	}
}
