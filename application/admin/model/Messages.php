...
class Messages extends Base{
	...
	public function pageQuery(){
	    ...
	    foreach ($rs['Rows'] as $key => $v){
         	$rs['Rows'][$key]['msgContent'] = MBISMSubstr(strip_tags($v['msgContent']),0,140);
        }
		return $rs;
	}
	...
	public function add(){
		...
		$data['sendUserId'] = session('MBIS_STAFF.staffId');
		//判断发送对象
		...
		MBISUnset($data,'id,sendType,htarget');//删除多余字段
		$list = [];
		//去重
		array_unique($ids);
		foreach($ids as $v)
