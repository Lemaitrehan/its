...
class Goods extends Base{
	...
	public function saleByPage(){
		...
		foreach ($rs['Rows'] as $key => $v){
			$rs['Rows'][$key]['verfiycode'] = MBISShopEncrypt($v['shopId']);
	...
	*/
	public function illegal(){
		...
		if($illegalRemarks=='')return MBISReturn("请输入违规原因");
		//判断商品状态
		...
		if((int)$rs['goodsId']==0)return MBISReturn("无效的商品");
		if((int)$rs['goodsStatus']!=1)return MBISReturn("操作失败，商品状态已发生改变，请刷新后再尝试");
		Db::startTrans();
		try{
			...
			if($res!==false){
				Db::name('carts')->where(['goodsId'=>$id])->delete();
				//发送一条商家信息
				MBISSendMsg($rs['userId'],"您的商品".$rs['goodsName']."【".$rs['goodsSn']."】因【".$illegalRemarks."】被下架处理。",['from'=>2,'dataId'=>$id]);
				Db::commit();
	...
	*/
	public function allow(){	
		...
		try{
			...
			if($res!==false){
				//发送一条商家信息
				MBISSendMsg($rs['userId'],"您的商品".$rs['goodsName']."【".$rs['goodsSn']."】已审核通过。",['from'=>2,'dataId'=>$id]);
				Db::commit();
				return MBISReturn('操作成功',1);
			}
		}catch (\Exception $e) {
	...
	public function searchQuery(){
		...
		$goodsCatIds = MBISGoodsCatPath($goodsCatatId);
		$key = input('post.key');
		$where = [];
		$where['g.dataFlag'] = 1;
		$where['g.isSale'] = 1;
