...
class Settlements extends Base{
    ...
    public function pageUnSettledQuery(){
    	...
    	$where['shopId'] = (int)session('MBIS_USER.shopId');
        ...
        if(count($page['Rows'])){
            foreach ($page['Rows'] as $key => $v) {
                $page['Rows'][$key]['payTypeNames'] = MBISLangPayType($v['payType']);
            }
        }
        return $page;
    }
    ...
    public function settlement(){
		...
		try{
            ...
            if(false !==  $result){
                 ...
                 Db::execute($upSql);
            	 Db::commit();
            	 return MBISReturn('提交结算申请成功，请留意结算信息~',1);
            }
		}catch (\Exception $e) {
            print_r($e);
            Db::rollback();
        }
        return MBISReturn('提交结算申请失败',-1);
    }
    ...
    public function pageSettledQuery(){
        ...
        $where['dataFlag'] = 1;
        $where['orderStatus'] = 2;
        $where['o.shopId'] = (int)session('MBIS_USER.shopId');
