...
class Goods extends Base{
	...
	public function saleByPage(){
		...
		foreach ($rs['Rows'] as $key => $v){
			$rs['Rows'][$key]['verfiycode'] = MBISShopEncrypt($shopId);
		}
		return $rs;
	}
	/**
	...
	public function add(){
		...
		MBISUnset($data,'goodsId,statusRemarks,goodsStatus,dataFlag');
		if(MBISConf("CONF.isGoodsVerify")==1){
			$data['goodsStatus'] = 0;
		}else{
			$data['goodsStatus'] = 1;
		}
        ...
        try{
			...
			if(false !== $result){
				...
				//商品图片
				MBISUseImages(0, $goodsId, $data['goodsImg']);
				//商品相册
				MBISUseImages(0, $goodsId, $data['gallery']);
				//商品描述图片
				MBISEditorImageRocord(0, $goodsId, '',$data['goodsDesc']);
    	        ...
    	        if($specsIds!=''){
		    		...
		    		foreach ($specsArray as $v){
		    			...
		    			if($sitem['itemImg']!='')MBISUseImages(0, $itemId, $sitem['itemImg']);
		    			$specMap[$v] = $itemId;
		    		}
		    		//保存销售规格
		    		$defaultPrice = 0;//最低价
		    		...
		    		if(!$isFindDefaultSpec)return MBISReturn("请选择推荐规格");
	...
	public function edit(){
	    ...
	    $specsIds = input('post.specsIds');
		$data = input('post.');
		MBISUnset($data,'goodsId,dataFlag,statusRemarks,goodsStatus,createTime');
        ...
        try{
        	//商品图片
			MBISUseImages(0, $goodsId, $data['goodsImg'],'goods','goodsImg');
			//商品相册
			MBISUseImages(0, $goodsId, $data['gallery'],'goods','gallery');
			// 商品描述图片
	        $desc = $this->where('goodsId',$goodsId)->value('goodsDesc');
			MBISEditorImageRocord(0, $goodsId, $desc, $data['goodsDesc']);
    	        ...
    	        if($specsIds!=''){
		    		...
		    		foreach ($specsArray as $v){
		    			...
		    			if(isset($specNameMap[$specNumId]) && (int)$specNameMap[$specNumId]!=0){
		    				$sitem['dataFlag'] = 1;
		    				MBISUseImages(0, (int)$specNameMap[$specNumId], $sitem['itemImg'],'spec_items','itemImg');
	...
	public function getById($goodsId){
		$rs = $this->where(['shopId'=>(int)session('MBIS_USER.shopId'),'goodsId'=>$goodsId])->find();
		if(!empty($rs)){
	 ...
	 */
     public function getBySale($goodsId){
		...
		if(!empty($rs)){
			$rs['read'] = false;
			//判断是否可以公开查看
			$viKey = MBISShopEncrypt($rs['shopId']);
			...
			$rs['scores']['totalScores'] = ($rs['scores']['totalScore']==0)?5:MBISScore($rs['scores']['totalScore'],$rs['scores']['totalUsers'],5,0,3);
			MBISUnset($rs, 'totalUsers');
			//关注
			$f = model('Favorites');
	...
	public function del(){
		...
		try{
	        ...
	        if(false !== $result){
	        	MBISUnuseImage('goods','goodsImg',$id);
	        	MBISUnuseImage('goods','gallery',$id);
	        	// 商品描述图片
	        	$desc = $this->where('goodsId',$id)->value('goodsDesc');
				MBISEditorImageRocord(0, $id, $desc,'');
				Db::commit();
	        	//标记删除购物车
	 ...
	 public function batchDel(){
		...
		try{
			...
			if(false !== $rs){
				//标记删除购物车
				foreach ($ids as $v){
					MBISUnuseImage('goods','goodsImg',(int)$v);
	        	    MBISUnuseImage('goods','gallery',(int)$v);
	        	    // 商品描述图片
		        	$desc = $this->where('goodsId',(int)$v)->value('goodsDesc');
					MBISEditorImageRocord(0, (int)$v, $desc,'');
				}
				Db::commit();
	        	return MBISReturn("删除成功", 1);
	        }
		...
		if($isSale==1){
			...
			//0.核对店铺状态
	 		...
	 		if($shopRs['shopStatus']!=1){
	 			return 	MBISReturn('上架商品失败!您的店铺权限不能出售商品，如有疑问请与商城管理员联系。',-3);
	 		}
	 		//直接设置上架 返回受影响条数
	 		$where = [];
			...
			if($rs!==false){
				...
				if($status==1){
					return MBISReturn('商品上架成功', 1,['num'=>$rs]);
				}else{
					return MBISReturn('已成功上架商品'.$rs.'件，请核对未能上架的商品信息是否完整。', 2,['num'=>$rs]);
				}
			}else{
	 			return MBISReturn('上架失败，请核对商品信息是否完整!', -2);
	 		}

		}else{
			$rs = $this->where(['goodsId'=>['in',$ids]])->setField('isSale',$isSale);
			if($rs !== false){
				return MBISReturn('商品上架成功', 1);
			}else{
				return MBISReturn($this->getError(), -1);
			}
		}
	...
	*/
	...
	public function changeGoodsStatus(){
		...
		$is = input('post.is');
		if(!in_array($is,$allowArr))return MBISReturn('非法操作',-1);
		//设置哪一个状态
	...
	public function checkExistGoodsKey($key,$val,$id = 0){
		if(!in_array($key,array('goodsSn','productNo')))return MBISReturn("非法的查询字段");
		$conditon = [$key=>$val];
	...
	public function stockByPage(){
		...
		$rsRows = Db::query($sql);
		$rs = MBISPager((int)$rsNum[0]['wstNum'],$rsRows,$page,$pageSize);
