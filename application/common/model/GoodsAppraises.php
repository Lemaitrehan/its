...
class GoodsAppraises extends Base{
	...
	*/
	public function add(){
		...
		if(empty($orders))return MBISReturn("无效的订单");
		if($orders['orderStatus']!=2)return MBISReturn("订单状态已改变，请刷新订单后再尝试!");
		//检测商品是否已评价
		$apCount = $this->where(['orderId'=>$orderId,'goodsId'=>$goodsId,'goodsSpecId'=>$goodsSpecId])->count();
		if($apCount>0)return MBISReturn("该商品已评价!");
		Db::startTrans();
		try{	
			//增加订单评价
			$data = [];
			...
			if($rs !==false){
				MBISUseImages(0, $this->id, $data['images']);
				//增加商品评分
				...
				//订单商品全部评价完则修改订单状态
				if($isFinish){
					if(MBISConf("isAppraisesScore")==1){
						//给用户增加积分
						$score = [];
						$score['userId'] = $userId;
						$score['score'] = 5;
	...
	*/
	public function shopReply(){
		...
		if($rs !== false){
			return MBISReturn('回复成功',1);
		}else{
			return MBISReturn('回复失败',-1);
		}
